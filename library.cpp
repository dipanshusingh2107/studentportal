...
void library::search_book(string name)
{
    sqlite3 * db;
    int exit = sqlite3_open("admin.db",&db);
    string sql = "select count(*) from books where book_name = '"+name+"' and issuedto = 'NULL';" ;
    int count;
    sqlite3_exec(db, sql.c_str(), countcallback, &count, NULL);
    cout<<"Number Of Books Availabe are: "<<count<<endl;
}
